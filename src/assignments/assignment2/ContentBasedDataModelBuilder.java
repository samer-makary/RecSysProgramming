...
import assignments.dao.ItemTagDAO;
...
/**
 * Builder for computing {@linkplain ContentBasedDataModel TF-IDF models} from item tag data.  Each item is
 * represented by a normalized TF-IDF vector.
 *
 * @author <a href="http://www.grouplens.org">GroupLens Research</a>
 */
public class ContentBasedDataModelBuilder implements Provider<ContentBasedDataModel> {
     ...
     */
    @Inject
    public ContentBasedDataModelBuilder(@Transient ItemTagDAO dao) {
        this.dao = dao;
    }
     ...
     */
    @Override
    public ContentBasedDataModel get() {
        ...
        for (VectorEntry e: docFreq.fast()) {
        	...
        	docFreq.set(e, Math.log(items.size() / e.getValue()));
        }
        ...
        return new ContentBasedDataModel(tagIds, modelData);
    }
