...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        // SVD will decompose a matrix with dimensions (m X n)
        // where m is the number of users and n is the number of items
        SingularValueDecomposition svd = new SingularValueDecomposition(matrix);

        // Third, truncate the decomposed matrix
        // notice that RealMatrix objects are 0-based
        final int m = userMapping.size() - 1;
        final int n = itemMapping.size() - 1;
        final int f = featureCount - 1;

        return new SVDModel(userMapping, itemMapping, 
                svd.getU().getSubMatrix(0, m, 0, f),	// (m X f) matrix
                svd.getV().getSubMatrix(0, n, 0, f),	// (n X f) matrix
                svd.getS().getSubMatrix(0, f, 0, f));	// (f X f) matrix
    }
    ...
    private RealMatrix createRatingMatrix(IdIndexMapping userMapping, IdIndexMapping itemMapping) {
        ...
        try {
            for (UserHistory<Event> user: users) {
                ...
                // subtract the baseline scores from the rating of the user
                ratings.subtract(baselines);
                
                // fill in the matrix row for this user will the normalized ratings
                for (VectorEntry ve: ratings.fast()) {
                	matrix.setEntry(u, itemMapping.getIndex(ve.getKey()), ve.getValue());
                }
            }
        } finally {
            users.close();
